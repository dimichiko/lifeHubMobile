...
export const HabitsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [dreamLogs, setDreamLogs] = useState<{ [habitId: string]: { [dayIdx: number]: string } }>({});
  ...
  const editHabit = (habit: Habit) => {
    setHabits((prev: Habit[]) => prev.map(h => h.id === habit.id ? habit : h));
  };
  ...
  return (
    <HabitsContext.Provider value={{ habits, progress, setCheck, addHabit, editHabit, removeHabit, dreamLogs, saveDream, carpetas, addCarpeta }}>
      {children}
